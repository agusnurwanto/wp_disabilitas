<?php
    $body = "";
    $path = WP_DISABILITAS_PLUGIN_PATH.'/sql-migrate';
    ...
    krsort($data);
    $body = implode('', $data);
    $last_update = get_option('_last_update_sql_migrate_disabilitas');
    ...
    if($versi !== $this->version){
        $ket = '
            <div class="notice notice-warning is-dismissible">
                <p>Versi database WP-Disabilitas tidak sesuai! harap dimutakhirkan. Versi saat ini=<b>'.$this->version.'</b> dan versi WP-Disabilitas kamu=<b>'.$versi.'</p>
            </div>
         ';
    }else{
        $ket = '
            <div class="notice notice-warning is-dismissible">
                <p>Versi database WP-Disabilitas sudah yang terbaru! Versi=<b>'.$this->version.'</b></p>
            </div>
         ';
    }
?>
...
<div class="cetak">
    <div style="padding: 10px;">
        <input type="hidden" value="<?php echo get_option( WP_DISABILITAS_KEY ); ?>" id="api_key">
        <h1 class="text-center">Monitoring SQL migrate WP-Disabilitas</h1>
...
<script type="text/javascript">
    function run_sql_migrate(file) {
        ...
        jQuery.ajax({
            ...
            type: "post",
            data: {
                "action": "run_sql_migrate_disabilitas",
            ...
            success: function(data){
                if(data.status == 'success'){
                    ...
                    jQuery(".notice").html('<p>Versi database WP-Disabilitas sudah yang terbaru! Versi=<b>'+data.version+'</b></p>');
                    update_status();
                }else{
